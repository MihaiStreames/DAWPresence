...
static ICON_GREEN_RGBA: LazyLock<(Vec<u8>, u32, u32)> =
    LazyLock::new(|| decode_icon(ICON_GREEN_DATA));
...
pub(crate) fn load_window_icon() -> Result<window::Icon, TrayError> {
    let (rgba, width, height) = cached_rgba(false);
    window::icon::from_rgba(rgba, width, height).map_err(|e| TrayError::IconFailed(e.to_string()))
}
